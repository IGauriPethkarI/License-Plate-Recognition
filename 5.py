...
def plateRecognition(imgOrignal):
    ...
    cv2.imwrite("GrayScaleImage.jpg", imgGray)
    cv2.waitKey(0)

    #Reduce noise and make the image smooth
    imgSmooth = cv2.bilateralFilter(imgGray, 11, 17, 17)
    cv2.imshow("Smooth Gray Image", imgSmooth)
    cv2.imwrite("SmoothGrayImage.jpg", imgSmooth)
    cv2.waitKey(0)

    #Find Edges
    imgEdged = cv2.Canny(imgSmooth, 170, 200)
    cv2.imshow("Canny Image", imgEdged)
    cv2.imwrite("CannyImage.jpg", imgEdged)
    cv2.waitKey(0)
    ...
    cv2.imwrite("ThresholdImage.jpg", imgThreshold)
    cv2.waitKey(0)
    ...
    cv2.imwrite("ContoursImage.jpg", imgContour)
    cv2.waitKey(0)

    #Area Classification (Top 30 contours)
    contours = sorted(contours, key= cv2.contourArea, reverse= True)[:5]
    NumberPlate = None

    imgDefined = img.copy()
    cv2.drawContours(imgDefined, contours, -1, (255,51,25), 2)
    cv2.imshow("Defined Contours Image", imgDefined)
    cv2.imwrite("DefinedContoursImage.jpg", imgDefined)
    cv2.waitKey(0)
    ...
    cv2.imshow("Bounding Rectangle Image", imgBoundRect)
    cv2.imwrite("BoundingRectangleImage.jpg", imgBoundRect)
    cv2.waitKey(0)
    ...
    try:
        print("Car belongs to the state", States[stat])
    except:
    ...
    cv2.putText(imgResult, Text, (x,y-10), cv2.FONT_ITALIC, 0.75, (0,0,0),2)
    ...
    imgStack = stackImages(0.8,[[img, imgSmooth, imgEdged, imgThreshold], [imgContour, imgDefined, imgBoundRect, imgResult ]])
